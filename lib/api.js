...
export async function upsertBroadcast(yt, { resource, streamId }) {
  ...
  if (existing) {
    id = existing.id;
    const { data } = await yt.liveBroadcasts.update({
      part: ['snippet', 'status', 'contentDetails'],
  ...
  return {
    ...
    // What YouTube RETURNED, not what was sent (intent and result diverged on Day 2).
    privacy: returned?.status?.privacyStatus ?? 'unknown',
  };
}
...
export async function verify(yt, streamId) {
  ...
  const checks = buildVerifyChecks({ streamId, bound });
...
export async function status(yt) {
  for (const b of await listMyBroadcasts(yt)) {
    console.log(formatBroadcastLine(b));
  }
}
